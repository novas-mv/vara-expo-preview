...
/* ONE copy of the element model, in pages/home/. This page is a motion
   record, not a second implementation — it imports the same buildElement,
   studioEnv and drawOn the hero uses, so the thing being demonstrated can
   never drift from the thing that ships. */
import { buildElement, studioEnv, drawOn } from '../home/vara-element.js';
...
function init(){
  ...
  const IN_DIST = 0.42, OUT_DIST = 0.95;    // world units, at rest scale
  ...
  const F_IN = 1.25, F_HOLD = 2.05, F_OUT = 0.80, F_GAP = 0.30;
  ...
  const flyStagger = { coral:0, green:0.13, violet:0.26, cyan:0.38 };
  ...
  const FK = { grow:0, off:0, big:1, spin:0, erase:0, vis:true };
  function flyK(name){
    let t = (loopT - flyStagger[name]) % FLY_LOOP;
    if (t < 0) t += FLY_LOOP;

    if (t < F_IN){
      const u = t / F_IN;
      /* The three beats of the storyboard, in order.
         1. DOT — holds for a moment, in frame, small.
         2. DRAW — it stretches along its own spine into the full ribbon...
         3. ...while SWELLING oversized and tumbling, then contracting into
            the lockup. The swell peaks in the middle of the draw, so what you
            watch is a huge stroke building itself and then collapsing to
            size — which is the whole feeling of the reference. */
      FK.grow = sstep(0.12, 0.78, u);
      FK.off  = IN_DIST * (1 - sstep(0, 1, u));
      /* Up then down: rises to PEAK across the draw, falls back to 1 as it
         settles. A single ease-down (huge -> small) reads as a zoom; the
         bump reads as the stroke inflating into existence. */
      const bump = sstep(0.04, 0.46, u) * (1 - sstep(0.56, 1.0, u));
      FK.big  = 1 + (PEAK - 1) * bump;
      FK.spin = SPIN_IN * (1 - sstep(0.12, 1.0, u));
      FK.erase = 0;
      FK.vis  = true;
    } else if (t < F_IN + F_HOLD){
      FK.grow = 1; FK.off = 0; FK.big = 1; FK.spin = 0; FK.erase = 0; FK.vis = true;
    } else if (t < F_IN + F_HOLD + F_OUT){
      const raw = (t - F_IN - F_HOLD) / F_OUT, u = inCubic(raw);
      FK.grow = 1;
      FK.off  = OUT_DIST * u;
      FK.big  = 1 + (BIG_OUT - 1) * u;  // swells toward camera as it goes
      FK.spin = SPIN_OUT * u;
      /* The exit MORPHS, it does not just travel. The stroke is eaten along
         its own spine from one end — the same drawOn window that built it,
         run as a wipe instead of a reveal — so it is consumed down to a
         travelling point and gone. Accelerating, so it whips out rather than
         dissolving politely. A stroke that only translates reads as a static
         object being moved, which is exactly what it looked like. */
      /* Mildly accelerating, NOT double-eased: a cubic-on-smoothstep left the
         first quarter of the exit doing visibly nothing. The erase LEADS the
         travel — it starts collapsing before it starts moving — so the exit
         reads as the stroke consuming itself, with the move following. */
      FK.erase = Math.pow(raw, 1.55);
      FK.vis  = FK.erase < 0.995;
    } else {
      FK.grow = 0; FK.off = IN_DIST; FK.big = 1; FK.spin = SPIN_IN; FK.erase = 0;
      FK.vis  = false;                  // gone; the next cycle reopens as a dot
    }
    return FK;
  }
  ...
  window.__varaFly = () => ({ loopT:+loopT.toFixed(2), LOOP_ON, FLY_MODE,
    s: Object.entries(strokes).map(([n,m])=>({ n, vis:m.visible,
      p: m.userData.pv.position.toArray().map(v=>+v.toFixed(2)),
      sc: +m.userData.pv.scale.x.toFixed(2), g: +flyK(n).grow.toFixed(2), e: +flyK(n).erase.toFixed(2) })) });
  function pose(now){
    const T = clock(now);
    ...
    for (const [name, m] of Object.entries(strokes)){
      ...
      if (flying){
        ...
        m.visible = true;
        pv.position.set(rest.x + dir[0]*fk.off,
                        rest.y + dir[1]*fk.off,
